...
const MapLabelingTest = () => {
  ...
  const handleAnswerChange = (questionNumber, value) => {
    // Only allow single letter input and convert to uppercase
    const formattedValue = value.replace(/[^a-zA-Z]/g, '').toUpperCase().slice(0, 1);
    setUserAnswers(prev => ({
      ...prev,
      [questionNumber]: formattedValue
    }));
  };
  ...
  const calculateScore = () => {
    let correct = 0;
    testData.testData.questions.forEach(question => {
      const userAnswer = userAnswers[question.questionNumber]?.toUpperCase();
      const correctAnswer = testData.testData.correctAnswer[question.questionNumber.toString()]?.toUpperCase();
  ...
  const handleNewTest = async () => {
    ...
    try {
      ...
      const newTestCode = `I-T${randomTestNumber}`;
      ...
      navigate('/ielts/map-labeling/test', { 
        state: { 
          testData: {
        ...
        <div className="container mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between h-16">
            ...
            <nav className="hidden md:flex items-center space-x-8">
              ...
              <span className="text-white/80 font-medium">Map Labeling Test</span>
            </nav>
        ...
        <div className="container mx-auto px-4 max-w-7xl">
          ...
          <div className="bg-white/10 backdrop-blur-lg rounded-3xl p-6 shadow-xl border border-white/20 mb-8">
            <div className="flex items-center space-x-4 mb-4">
              <div className="flex-shrink-0 w-12 h-12 bg-lime-600 rounded-xl flex items-center justify-center">
              ...
              <div>
                ...
                <p className="text-white/70 text-sm">Listen carefully and label the map locations below</p>
              </div>
            </div>
            ...
            <div className="space-y-4">
              ...
              <div className="flex items-center space-x-4">
                <button
                  ...
                  className="flex items-center justify-center w-12 h-12 bg-lime-600 hover:bg-lime-700 rounded-full transition-colors duration-300"
                >
              ...
              >
                <div 
                  className="h-full bg-lime-500 rounded-full transition-all duration-100"
          ...
          {/* Main Content - Location Labeling */}
          <div className="bg-white/10 backdrop-blur-lg rounded-3xl p-8 shadow-xl border border-white/20">
            <h2 className="text-3xl font-bold text-white mb-8 text-center flex items-center justify-center">
              <span className="material-icons mr-3 text-lime-400 text-3xl">map</span>
              Label the Locations
            </h2>

            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              {testData.testData.questions.map((question) => (
                <div key={question.questionNumber} className="bg-white/5 rounded-xl p-6">
                  {/* Location Name */}
                  <div className="flex items-start space-x-4 mb-4">
                    <div className="flex-shrink-0 w-12 h-12 bg-lime-600 rounded-full flex items-center justify-center">
                  ...
                  <div className="flex items-center justify-center space-x-3">
                    <span className="text-white/70 text-sm font-medium">Letter:</span>
                    <input
                      type="text"
                      ...
                      placeholder="A"
                      maxLength="1"
                      className={`w-16 h-16 text-center text-2xl font-bold rounded-xl border-2 transition-all duration-300 ${
                        isSubmitted
                          ? userAnswers[question.questionNumber]?.toUpperCase() === testData.testData.correctAnswer[question.questionNumber.toString()]?.toUpperCase()
                            ? 'bg-green-100 border-green-500 text-green-800'
                            : 'bg-red-100 border-red-500 text-red-800'
                          : 'bg-white/90 border-white/30 text-gray-800 focus:border-lime-500 focus:bg-white'
                      }`}
                    />
                  </div>
                      ...
                      {userAnswers[question.questionNumber]?.toUpperCase() === testData.testData.correctAnswer[question.questionNumber.toString()]?.toUpperCase() ? (
                        ...
                        <div className="space-y-2">
                          ...
                          <div className="text-center">
                            <span className="text-white/70 text-sm">Correct answer: </span>
          ...
          {/* Results Section */}
          {showResults && (
            <div className="bg-white/10 backdrop-blur-lg rounded-3xl p-8 shadow-xl border border-white/20 mb-8 mt-8">
              <div className="text-center">
                ...
                <div className="flex justify-center space-x-4">
                  <button
                    ...
                    className="bg-lime-600 hover:bg-lime-700 disabled:bg-gray-600 disabled:cursor-not-allowed text-white px-6 py-3 rounded-xl font-semibold transition-colors duration-300 flex items-center space-x-2"
                  >
                    {isLoadingNewTest ? (
                      <>
                        <svg className="animate-spin h-4 w-4" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
          ...
          {!isSubmitted && (
            <div className="text-center mt-8">
              <button
                ...
                className="bg-gradient-to-r from-lime-600 to-green-600 hover:from-lime-700 hover:to-green-700 text-white px-8 py-4 rounded-2xl font-semibold text-lg transition-all duration-300 transform hover:scale-105 shadow-xl hover:shadow-2xl"
              >
                Submit Answers
              </button>
            </div>
...
export default MapLabelingTest; 