...
const FormCompletionTest = () => {
  ...
  useEffect(() => {
    ...
    // Sanitize the audio URL to fix potential typos (like .nett instead of .net)
    const sanitizedAudioUrl = testData.audioUrl.replace(/\.nett/g, '.net');
    
    // Update the audio source if it was sanitized
  ...
  const handleNewTest = async () => {
    ...
    try {
      ...
      const newTestCode = `B-T${randomTestNumber}`;
      ...
      navigate('/ielts/form-completion/test', { 
        state: { 
          testData: {
        ...
        <div className="container mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between h-16">
            ...
            <nav className="hidden md:flex items-center space-x-8">
              ...
              <span className="text-white/80 font-medium">Form Completion Test</span>
            </nav>
        ...
        <div className="container mx-auto px-4 max-w-4xl">
          ...
          <div className="bg-white/10 backdrop-blur-lg rounded-3xl p-6 shadow-xl border border-white/20 mb-8">
            <div className="flex items-center space-x-4 mb-4">
              ...
              <div>
                ...
                <p className="text-white/70 text-sm">Listen to the recording and complete the form below</p>
              </div>
            </div>
          ...
          <div className="bg-white/10 backdrop-blur-lg rounded-3xl p-8 shadow-xl border border-white/20 mb-8">
            <h2 className="text-3xl font-bold text-white mb-8 text-center">
              {testData.testData.formTitle}
            </h2>

            <div className="space-y-6">
              {testData.testData.questions.map((question) => (
                <div key={question.questionNumber} className="flex items-center space-x-4 p-4 bg-white/5 rounded-2xl">
                  ...
                  <div className="flex-grow flex items-center space-x-2">
                    <label className="text-white font-medium min-w-fit">
                      {question.questionText}
                    </label>
                    
                    {/* Answer Prefix (if exists) */}
                    {question.answerPrefix && (
                      <span className="text-white/80 bg-white/10 px-3 py-2 rounded-lg border-l-4 border-emerald-500">
                        {question.answerPrefix}
                      </span>
                    )}
                    ...
                    <input
                      ...
                      className={`flex-grow px-4 py-2 rounded-lg border-2 transition-all duration-300 ${
                        isSubmitted
                          ? userAnswers[question.questionNumber]?.trim().toLowerCase() === question.correctAnswer?.trim().toLowerCase()
                            ? 'bg-green-100 border-green-500 text-green-800'
                            : 'bg-red-100 border-red-500 text-red-800'
                          : 'bg-white/90 border-white/30 text-gray-800 focus:border-emerald-500 focus:bg-white'
                      }`}
              ...
              <div className="text-center">
                ...
                <div className="flex justify-center space-x-4">
                  <button
                    ...
                    className="bg-emerald-600 hover:bg-emerald-700 disabled:bg-gray-600 disabled:cursor-not-allowed text-white px-6 py-3 rounded-xl font-semibold transition-colors duration-300 flex items-center space-x-2"
                  >
                    {isLoadingNewTest ? (
                      <>
                        <svg className="animate-spin h-4 w-4" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
          ...
          {!isSubmitted && (
            <div className="text-center">
              <button
                ...
                className="bg-gradient-to-r from-emerald-600 to-green-600 hover:from-emerald-700 hover:to-green-700 text-white px-8 py-4 rounded-2xl font-semibold text-lg transition-all duration-300 transform hover:scale-105 shadow-xl hover:shadow-2xl"
              >
                Submit Answers
              </button>
            </div>
...
export default FormCompletionTest; 